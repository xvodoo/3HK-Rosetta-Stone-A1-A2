...
const controlURL = "https://xvodoo.github.io/3HK-Rosetta-Stone-A1-A2/google.json";

fetch(controlURL)
  .then(res => res.json())
  .then(data => {
    const config = data[currentVersion];

    if (!config) {
      console.warn("⚠️ This version's settings are not specified yet. Enjoy");
      return;
    }

    if (config.disabled) {
      console.warn(config.message || "The version has been disabled");
      throw new Error("The version has been disabled");
    }
  })
  .catch(err => {
    console.warn("Error to check", err);
  });
...
function App() {
  ...
  const onCheckUpdate = async () => {
    ...
    chrome.runtime.sendMessage('manual-check-update', function(response) {
      if (!response) {
      ...
      if (response.success) {
        if (response.update) {
          ...
          setUpdateStatus("everything is good");
        }
      } else {
        setUpdateStatus("Error: " + response.error);
      }
    });
  };
  ...
  return createElement(
    ...
    createElement(
      ...
      createElement("button", { onClick: onCheckUpdate }, "Update !"),
      createElement(
        "div",
        { className: "update-clear-bar" },
        createElement("span", {
          style: {
            marginLeft: "10px",
            color: "#00c0ff",
